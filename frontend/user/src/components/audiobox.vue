<template>
  <div id="audiobox">
    <div class="left">
      <div class="top">
        <b>{{index+1}}.[单选]&nbsp;分数:{{num}}分&nbsp;是否必做:{{isNec?"是":"否"}}&nbsp;选项乱序:{{isRan?"是":"否"}}&nbsp;</b>
      </div>
      <div class="bottom">
...
export default {
  ...
  updated () {
    ...
    this.$emit('Updatedata', updatedata, this.index, 0)
  },
  methods: {
    del (id) {
...
#audiobox .right .upload-demo {
  transform: scale(90%);
}
#audiobox .right .point {
  display: flex;
